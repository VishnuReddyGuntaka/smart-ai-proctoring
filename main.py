...
class_labels = open('model/modellabels').read().strip().split('\n')

# Reading CNN model (YOLO)
cnn_model = cv2.dnn.readNetFromDarknet('model/model.cfg', 'model/model.weights')
...
def webcamVideo():
    ...
    while True:
        ...
        for i in ids.flatten():
            ...
            if class_ids[i] == 0:  # Person detected
                pose = getPose(frame)
...
exitButton = Button(main, text="Exit", command=exitApp)
